...
export default function CustomersPage() {
  ...
  const [form, setForm]               = useState({
    name:'', phone:'', email:'', address:'', balance:0, isReceivable:true
  });
  ...
  const fetchData = async () => {
    try {
      ...
      setCustomers(data.map((c: any) => ({
        ...
        status: c.status === 'active' ? 'active' : 'payable',
      })));
    } catch (e) {
  ...
  const handleAdd = async () => {
    if (!form.name || form.phone.length !== 10) {
      return alert('Name and 10‑digit phone required');
    }
    const status = form.isReceivable ? 'active' : 'payable';
    try {
      const res = await fetch('http://localhost:3001/api/customers', {
        method:'POST',
      ...
      setShowModal(false);
      setForm({ name:'', phone:'', email:'', address:'', balance:0, isReceivable:true });
      await fetchData();
    } catch {
  ...
  const totalReceivable = customers.reduce((sum, c) => sum + (c.status==='active'?c.balance:0), 0);
                ...
                : list.map(c => (
                    ...
                    >
                      ...
                      <div className="text-end">
                        <Badge bg={c.status==='active'?'success':'danger'} className="mb-1">
              ...
              {selected
                ? <>
                    ...
                    <p>
                      ...
                      <Badge bg={selected.status==='active'?'success':'danger'}>
      ...
      <Modal show={showModal} onHide={()=>setShowModal(false)} centered>
        ...
        <Modal.Body>
          <Form>
            ...
            <Form.Check
              className="mt-3"
              inline label="Receivable" type="radio"
              checked={form.isReceivable}
              onChange={()=>setForm({...form,isReceivable:true})}
            />
            <Form.Check
              inline label="Payable" type="radio"
              checked={!form.isReceivable}
